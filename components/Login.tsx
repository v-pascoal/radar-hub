...
const Login: React.FC<LoginProps> = ({ initialRole = UserRole.CLIENT, onLoginSuccess, onCancel }) => {
  ...
  // Gera código alfanumérico de 4 dígitos (ex: 7C96, ABC1)
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    // Gera o código antes de mudar o estado
    const newOtp = generateAlphanumericOtp();
    setGeneratedOtp(newOtp);
    
    // Pequeno delay para UX, seguido do alerta
    setTimeout(() => {
      setLoading(false);
      setStep('otp');
      // Garante que o alerta apareça
      window.alert(`Radar Hub: Seu código de acesso é ${newOtp}`);
    }, 800);
  };
  ...
  const handleFinalize = (e: React.FormEvent) => {
    ...
    // Validação de segurança opcional ou bypass (conforme pedido: aceitando qualquer coisa por enquanto)
    // No entanto, ainda checamos se tem 4 caracteres para consistência visual
      ...
      <div className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[95vh] flex flex-col relative">
        <div className="p-8 md:p-12 overflow-y-auto">
          ...
          <div className="flex items-center mb-8">
            ...
            {/* Tabs - Só aparecem no Step 1 (Info) conforme pedido */}
            {step === 'info' && (
          ...
          <div className="mb-10 text-center">
            <h2 className="text-[2.6rem] font-black text-[#1e1b4b] italic uppercase tracking-tighter mb-1 leading-none">
              BOM TE VER DE <br />
              <span className="text-[#593EFF]">VOLTA</span>
            </h2>
          ...
          {step === 'info' ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input required placeholder="(00) 00000-0000" className="w-full pl-16 pr-7 py-6 bg-white border-2 border-[#593EFF] rounded-[1.8rem] outline-none font-black text-[#1e1b4b]" value={formData.phone} onChange={e => handleInputChange('phone', e.target.value)} />
                  </div>
                </div>
              </div>

              <div className="flex flex-col gap-4">
                <button type="submit" disabled={loading} className="w-full py-6 bg-[#593EFF] text-white font-black rounded-[1.8rem] shadow-[0_15px_30px_rgba(89,62,255,0.4)] hover:scale-[1.02] active:scale-95 transition-all uppercase text-[11px] tracking-widest">
                  {loading ? 'Carregando...' : mode === 'login' ? 'Entrar na conta' : 'Criar minha conta'}
                </button>
                {mode === 'login' && (
                  <button type="button" onClick={() => setMode('register')} className="w-full py-4 bg-slate-100 text-[#1e1b4b] font-black rounded-[1.5rem] uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-all">
                    Não tenho conta
                  </button>
                )}
              </div>
            ...
            /* PASSO 2: CÓDIGO DE VERIFICAÇÃO - UNIFICADO E SEM TABS */
            <form onSubmit={handleFinalize} className="space-y-10 text-center py-6 animate-in zoom-in-95 duration-300">
              ...
              <div className="flex justify-center">
                <input 
                  ...
                  autoFocus
                  autoComplete="off"
                  className="w-full max-w-[340px] h-28 px-6 bg-white border-2 border-[#593EFF]/20 rounded-[2.5rem] font-black text-5xl text-center tracking-[0.3em] text-[#593EFF] outline-none focus:border-[#593EFF] transition-all uppercase shadow-sm" 
                  value={formData.otp} 
              ...
              <div className="space-y-6 pt-4 flex flex-col items-center">
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full py-6 bg-[#593EFF] text-white font-black rounded-[1.8rem] uppercase text-[11px] tracking-widest shadow-2xl shadow-indigo-200 hover:scale-[1.02] active:scale-95 transition-all"
                >
